class ShowingConfirmedNotificationWorker
  ...
  def perform(showing_id)
    ...
    body = "Your showing request was confirmed: #{showing.address.single_line}.  For more details visit: #{users_buyers_requests_url}"
    log_msg = "Sending SMS showing confirmed notification to #{showing.user.full_name} (#{showing.user.profile.phone1}) - #{body}"
    Rails.logger.tagged("Showing Confirmed Notification SMS") { Rails.logger.info log_msg }
